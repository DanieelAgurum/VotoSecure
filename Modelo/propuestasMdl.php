...
class PropuestasMdl
{
    ...
    {
        $query = "SELECT * FROM {$this->tabla} ORDER BY id_propuesta DESC";
    ...
    {
        $query = "SELECT * FROM {$this->tabla} WHERE id_propuesta = ?";
        $stmt = $this->conexion->prepare($query);
    ...
    public function actualizar($id, $candidato_id, $titulo, $slogan, $mision, $propuesta_detallada, $video_url)
    {
        $query = "UPDATE {$this->tabla} 
                  SET candidato_id = ?, titulo = ?, slogan = ?, mision = ?, propuesta_detallada = ?, video_url = ?
                  WHERE id_propuesta = ?";
